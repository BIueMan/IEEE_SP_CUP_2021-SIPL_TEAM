...
SEED_LIST = (565, 121, 444, 2121, 5456, 4542, 456, 53, 452, 32, 112, 566, 129, 3455, 6562, 1287)
USER_LIST = range(50) # USER -1 (Python is shit)

EPOCHS_RAND_START_CONFIGE = 15
EPOCHS_CONFING = 50
LEARNING_RATE_DNN = 5e-2
LEARNING_RATE_CNN = 10**-2
EPOCHS_CONV = 10
if __name__ == "__main__":
    ...
    for seed in SEED_LIST:
        ...
        for userToCheck in USER_LIST: # Python is the best
            ...
            model_r_list.append(model_r1)

            model_r2 = torch.nn.Sequential(
                torch.nn.Conv2d(1, 4, kernel_size=(5, 5), stride=1, padding=(2, 2)),
                torch.nn.Conv2d(4, 16, kernel_size=(5, 5), stride=1, padding=(2, 2)),
                torch.nn.Tanh(),
                torch.nn.Conv2d(16, 4, kernel_size=(5, 5), stride=1, padding=(2, 2)),
                torch.nn.Conv2d(4, 1, kernel_size=(5, 5), stride=1, padding=(2, 2)),
                torch.nn.Tanh()
            ).to(device)
            model_r_list.append(model_r2)

            model_r3 = torch.nn.Sequential(
                torch.nn.Conv2d(1, 16, kernel_size=(9, 9), stride=1, padding=(4, 4)),
                torch.nn.Conv2d(16, 32, kernel_size=(9, 9), stride=1, padding=(4, 4)),
                torch.nn.Conv2d(32, 64, kernel_size=(9, 9), stride=1, padding=(4, 4)),
                torch.nn.Tanh(),
                torch.nn.Conv2d(64, 32, kernel_size=(9, 9), stride=1, padding=(4, 4)),
                torch.nn.Conv2d(32, 16, kernel_size=(9, 9), stride=1, padding=(4, 4)),
                torch.nn.Conv2d(16, 1, kernel_size=(9, 9), stride=1, padding=(4, 4)),
                torch.nn.Tanh()
            ).to(device)
            model_r_list.append(model_r3)

            for model_r in model_r_list:
